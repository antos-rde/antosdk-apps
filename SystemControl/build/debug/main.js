(function() {
  var SysmondService, SystemControl;

  SysmondService = class SysmondService extends OS.application.BaseService {
    constructor(args) {
      super("SysmondService", args);
      this.text = __("{0}%", 0..toString());
      this.iconclass = "fa fa-android";
      this.app = void 0;
      //@nodes = [
      //    {text: __("Status"), id: 1},
    ...
    init() {
      ...
      checklib = () => {
        if (!Antunnel.tunnel) {
          ...
          this.sub = new Antunnel.Subscriber("fbf070ddea3ea90d07f456540b405d302554ec82");
          this.sub.onopen = () => {
            //@sub.send Antunnel.Msg.DATA, new TextEncoder("utf-8").encode("Hello")
            return console.log("Subscribed to notification channel");
          };
          this.sub.onerror = (e) => {
          ...
          this.sub.onmessage = (e) => {
            var obj;
            if (e.data) {
              obj = JSON.parse(new TextDecoder("utf-8").decode(e.data));
            }
            // update the battery
            this.text = __("{0}%", Math.round(obj.battery_percent).toString());
            if (this.app) {
              this.app.feed(obj);
            }
            return this.update();
          ...
          this.sub.onclose = () => {
            this.sub = void 0;
            this.notify(__("Unsubscribed to the notification service"));
            return this.quit();
          };
    ...
    openApp() {
      ...
      return this._gui.launch("SystemControl", []);
    }
  ...
  SystemControl = class SystemControl extends this.OS.application.BaseApplication {
    constructor(args) {
      super("SystemControl", args);
    }

    main() {
      this.diskchart = $(this.find("disk-area")).epoch({
        type: 'time.gauge',
        value: 0
      });
      this.on("resize", () => {
        var el;
        el = this.find("cpu-area");
        if (this.cpu) {
          $(el).children().hide();
          this.cpu.option("width", $(el).width());
          this.cpu.option("height", $(el).height());
          $(el).children().show();
        }
        el = this.find("battery-area");
        el = this.find("memory-area");
        if (this.memory) {
          $(el).children().hide();
          this.memory.option("width", $(el).width());
          this.memory.option("height", $(el).height());
          $(el).children().show();
        }
        el = this.find("network-area");
        if (this.network) {
          $(el).children().hide();
          this.network.option("width", $(el).width());
          this.network.option("height", $(el).height());
          $(el).children().show();
        }
        el = this.find("temp-area");
        if (this.temp) {
          $(el).children().hide();
          this.temp.option("width", $(el).width());
          this.temp.option("height", $(el).height());
          $(el).children().show();
        }
        el = this.find("battery-area");
        if (this.battery) {
          $(el).children().hide();
          this.battery.option("width", $(el).width());
          this.battery.option("height", $(el).height());
          return $(el).children().show();
        }
      });
      return this._gui.pushService("SystemControl/SysmondService", []).then((p) => {
        this.service = p;
        return p.app = this;
      }).catch((e) => {
        return this.error(__("Unable to start sysmond service"), e);
      });
    }

    streamline(name, data, range, labels) {
      var dobj, i, item, j, k, legend, len, len1, options;
      i = 0;
      legend = $(this.find(`${name}-text`));
      if (!this[name]) {
        options = {
          type: 'time.line',
        ...
        for (j = 0, len = data.length; j < len; j++) {
          ...
          $("<div>").addClass('legend-color').addClass('ref').addClass('category' + (i + 1)).appendTo(legend);
          $("<div>").addClass('legend-label').appendTo(legend).text(labels ? labels[i] : `${name}-${i}`);
          $("<div>").addClass('legend-value').appendTo(legend);
          i = i + 1;
        }
        options.data = dobj;
        this[name] = $(this.find(`${name}-area`)).epoch(options);
      }
      ...
      return this[name].push(data);
    }

    feed(data) {
      var battery_range, cpu_data, disk_used, j, k, len, len1, mem_data, mem_range, net_data, net_rx, net_tx, now, ref, ref1, temp_data, total_mem, total_swap, used_mem, used_swap, v;
      ...
      this.diskchart.push(disk_used / data.disk_total);
      this.find("disk-text").text = "Disk: " + (Math.round(disk_used / 1024 / 1024 / 1024)) + "/" + (Math.round(data.disk_total / 1024 / 1024 / 1024)) + " GB";
      now = data.stamp_sec;
      ...
      this.streamline("cpu", cpu_data, [0, 100]);
      ...
      mem_data = [
        {
          time: now,
          y: used_mem.toFixed(3)
        },
        {
          time: now,
          y: used_swap.toFixed(3)
        }
      ];
      mem_range = [0, total_mem > total_swap ? total_mem : total_swap];
      this.streamline("memory", mem_data, mem_range, ["RAM (GB)", "SWAP (GB)"]);
      net_rx = 0;
      net_tx = 0;
      ref = data.net;
      for (j = 0, len = ref.length; j < len; j++) {
        v = ref[j];
        net_rx = net_rx + v.rx_rate;
      }
      ref1 = data.net;
      for (k = 0, len1 = ref1.length; k < len1; k++) {
        v = ref1[k];
        net_tx = net_tx + v.tx_rate;
      }
      net_data = [
        {
          time: now,
          y: (net_rx / 1024.0).toFixed(3)
        },
        {
          time: now,
          y: (net_tx / 1024.0).toFixed(3)
        }
      ];
      this.streamline("network", net_data, void 0, ["RX (Kb/s)", "TX (Kb/s)"]);
      temp_data = [
        {
          time: now,
          y: (data.cpu_temp / 1000.0).toFixed(2)
        },
        {
          time: now,
          y: (data.gpu_temp / 1000.0).toFixed(2)
        }
      ];
      this.streamline("temp", temp_data, void 0, ["CPU temp (C)", "GPU temp (C)"]);
      battery_range = [(data.battery_min_voltage / 1000.0).toFixed(2), (data.battery_max_voltage / 1000.0).toFixed(2)];
      return this.streamline("battery", [
        {
          time: now,
          y: (data.battery / 1000.0).toFixed(2)
        }
      ], battery_range, ["Baterry (v)"]);
    }
  ...
  SystemControl.dependencies = ["pkg://Antunnel/main.js"];
